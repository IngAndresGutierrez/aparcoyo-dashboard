import { DeleteReviewResponse, PlazasUsuarioResponse, UpdateReviewRequest, UpdateReviewResponse } from "../types/reseñas";
...
class PlazaReviewsService {
  ...
  async updateReview(
    ...
    console.log(`📤 Actualizando reseña ${reviewId} de plaza ${plazaId}:`, updateData);
    
    try {
      const response = await fetch(`${this.baseURL}/${plazaId}/reviews/${reviewId}`, {
        method: 'PATCH',
        headers: this.getHeaders(),
  ...
  async deleteReview(plazaId: string, reviewId: string): Promise<DeleteReviewResponse> {
